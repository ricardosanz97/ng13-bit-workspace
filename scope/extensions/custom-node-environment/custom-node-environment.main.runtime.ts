...
export class CustomNodeEnvironmentMain {
  static slots = [];
  ...
  static async provider([node, envs]: [NodeMain, EnvsMain]) {
    ...
    const CustomNodeEnvironmentEnv = node.compose([
      /**
    ...
    envs.registerEnv(CustomNodeEnvironmentEnv);
    return new CustomNodeEnvironmentMain();
  }
}

CustomNodeEnvironmentAspect.addRuntime(CustomNodeEnvironmentMain);
